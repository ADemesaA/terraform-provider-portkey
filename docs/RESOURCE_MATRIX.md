...
| `portkey_api_key` | ✅ | ✅ | ✅ | ✅ | ✅ | Full CRUD + on-demand `/rotate` (via `rotate_trigger`) | ✅ 30 tests |
...
| `portkey_scim_workspace_mapping` | ✅ | ✅ | ❌ | ✅ | ✅ | No PATCH; role/group/workspace changes ForceNew. `/v2/scim/workspaces` per support guidance while `/v1` routing is being fixed | ✅ Unit (client) + acceptance |
...
| `portkey_scim_workspace_mappings` | - | ✅ | Working (`GET /v2/scim/workspaces`) | ✅ 2 tests |
...
POST   /api-keys/{id}/rotate       → On-demand rotation (via `rotate_trigger`); requires the matching *_api_keys.update Admin scope
